...
public class ShopTypeServiceImpl extends ServiceImpl<ShopTypeMapper, ShopType> implements IShopTypeService {
    ...
    public Result queryShopList() {
        // 1. 从redis中查询商铺类型列表
        String jsonArray = stringRedisTemplate.opsForValue().get(SHOP_TYPE_LIST);
        // json转list
        ...
        // 4. 将商铺类型存入到redis中
        stringRedisTemplate.opsForValue().set(SHOP_TYPE_LIST,JSONUtil.toJsonStr(shopTypesByMysql));
        // 5. 返回数据库中商铺类型信息
